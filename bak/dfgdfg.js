...
async function main() {
 ...
 let t = await fs.copy(model_config_meta, model_config, {replace: true});
...
//console.log('sdfasdf')
  for (let model of models) {
    ...
    try {
      ...
      if (!exist) {
        await fs.writeFileSync(output_json, JSON.stringify(schema, null, 4))
    ...
    try {
      ...
      if (!exist) {
        fs.writeFileSync(output_javascript, jsFileString(model.name, null, 4))
  ...
  fs.outputJsonSync(model_config, merged, {spaces: 2});
  fs.copySync(replace_user_js, user_js, {replace: true});
